...
bool readingExistingAccBuffer(linalg::LinalgOp op) {
 ...
 // that benefits from being staged in shared memory before the K-loop.
 //
 // Excluded from "needs promotion":
 //   - tensor.empty / linalg.fill — uninitialized or zero, no values to read
 //   - bufferization.to_tensor of a function arg / global memref — the matmul
 //     can write directly to that destination's tile slice; staging it in
 //     shared just adds a redundant round trip and breaks GPU bufferization
 //     for the writable variant (FuseMatmulBias-fused matmul-accumulate).
 for (OpOperand &initOperand : op.getDpsInitsMutable()) {
   ...
   if (defOp && isa<bufferization::ToTensorOp>(defOp)) continue; // direct dest
