...
static bool tryHoistLabelDrivenExtract(IRRewriter &rewriter,
  ...
  // Add one identity map (same as output map) per new input.
  auto oldMaps = linalgOp.getIndexingMapsArray();
  SmallVector<AffineMap> newMaps(oldMaps);
  AffineMap outMap = oldMaps.back();
  for (size_t i = 0; i < extractOps.size(); ++i)
    newMaps.insert(newMaps.end() - 1, outMap);
