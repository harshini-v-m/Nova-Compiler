...
namespace mlir::nova
{
  ...
  // ── Step 10.5: Multi-buffer workgroup allocs (3 stages) ────────────────
  // Widens promoted A/B SMEM tiles to 3 buffers indexed by (iv mod 3) so the
  // K-loop body has independent SMEM regions per pipeline stage. Tags the
  // rewritten scf.for with a marker that NovaGPUPipelining gates on.
  ...
  // ── Step 12.6: Software-pipeline the K-loop ───────────────────────────
  // Time-shifts nvgpu.device_async_copy ops by depth-1 iterations so that
  // iteration i+2 prefetches into buffer (i+2)%3 while iteration i computes
  // on buffer i%3. Rewrites the wait_group count from wait-all (= 0) to
  // depth-1 (= 2). Pipelining is gated on the marker set by
  // NovaGPUMultiBuffering: loops without widened buffers are left alone to
