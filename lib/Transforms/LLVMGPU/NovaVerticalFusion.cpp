...
static LogicalResult lowerFusedReductionElemToSCF(linalg::GenericOp reduceOp,
  ...
  // ── Compute output shape for the bufferized result ────────────────────────
  // The output of the fused op has the parallel dims shape.
  SmallVector<int64_t> outShape;
  for (int64_t d : parallelDims)
    outShape.push_back(loopRanges[d]);

  auto initTensorType = cast<RankedTensorType>(initTensor.getType());
  Type elemType = initTensorType.getElementType();

  // Allocate a flat memref for the output so the forall body can store into it.
  // This is the same two-step pattern used by lowerFullReduceMeanToSCF:
  //   1. scf.forall (GPU-block mapped) over parallel dims
  //   2. scf.for (sequential) over reduction dims inside each block
  // After the forall, cast the memref back to a tensor so downstream SSA
  // values stay valid.
  auto outMemType = MemRefType::get(outShape, elemType);
  Value outMem = builder.create<memref::AllocOp>(loc, outMemType);

  // ── GPU mapping: one block per parallel-dim point ─────────────────────────
  // We assign gpu.block_id {x, y, z} to the first three parallel dims.
  // If there are more than 3 parallel dims, the extras are covered by dim-0
  // (the forall iterates linearly over all points in those dims together).
  // For the common 1- or 2-D parallel cases this is exact.
  SmallVector<Attribute> mappingAttrs;
  SmallVector<gpu::MappingId> gpuIds = {
      gpu::MappingId::DimX, gpu::MappingId::DimY, gpu::MappingId::DimZ};
  for (int64_t p = 0; p < std::min(numParallel, (int64_t)3); ++p)
    mappingAttrs.push_back(
        gpu::GPUBlockMappingAttr::get(builder.getContext(), gpuIds[p]));
  // If numParallel > 3, flatten the remainder into DimX by using a single
  // forall over the linearised product — handled below by emitting the
  // forall over the first `min(numParallel,3)` dims and looping the rest.
  ArrayAttr mappingAttr = builder.getArrayAttr(mappingAttrs);

  // Bounds / steps for the forall (parallel dims only).
  int64_t forallRank = std::min(numParallel, (int64_t)3);
  SmallVector<OpFoldResult> forallLBs(forallRank,
                                      getAsOpFoldResult(c0));
  SmallVector<OpFoldResult> forallUBs, forallSteps(forallRank,
                                                    getAsOpFoldResult(c1));
  for (int64_t p = 0; p < forallRank; ++p)
    forallUBs.push_back(getAsOpFoldResult(bounds[parallelDims[p]]));

  // Shared IV array: ivs[d] = loop IV for logical dimension d of reduceOp.
  ...
  // ── Outer: scf.forall over parallel dims (GPU block mapped) ──────────────
  //
  // We emit scf.forall instead of scf.for so that convert-scf-forall-to-gpu
  // (or iree-gpu-map-forall-to-gpu) can lower it to gpu.launch / gpu.func.
  // The forall body uses memref.store into outMem rather than tensor.insert,
  // because scf.forall with no shared_outputs uses in-place writes.
  auto forallOp = builder.create<scf::ForallOp>(
      loc, forallLBs, forallUBs, forallSteps,
      /*outputs=*/ValueRange{},
      mappingAttr);
  {
    ...
    // Bind the forall IVs to the first forallRank parallel dims.
    for (int64_t p = 0; p < forallRank; ++p)
      ivs[parallelDims[p]] = forallOp.getInductionVar(p);

    // If numParallel > 3, loop the remaining parallel dims sequentially inside
    // the forall body (rare; avoids a hard limit on 3 parallel dims).
    std::function<void(OpBuilder &, int64_t)> emitExtraParallelLoops;
    emitExtraParallelLoops = [&](OpBuilder &b, int64_t pidx) {
      if (pidx == numParallel) {
        // All parallel IVs set. Run the reduction + elementwise body.

        // Seed accumulator from the init tensor.
        SmallVector<Value> redOutIdx =
            evalAffineMap(b, loc, reduceOutputMap, ivs);
        Value initAcc =
            b.create<tensor::ExtractOp>(loc, initTensor, redOutIdx);

        // Run reduction.
        Value finalAcc = buildReductionLoops(b, 0, initAcc);

        // Inline the elementwise body.
        SmallVector<Value> elemIVs;
        for (int64_t d : parallelDims)
          elemIVs.push_back(ivs[d]);
        ...
        // Output init arg for the elementwise op (read current outMem value).
        SmallVector<Value> elemOutIdx =
            evalAffineMap(b, loc, elemOutputMap, elemIVs);
        Value outInit = b.create<memref::LoadOp>(loc, outMem, elemOutIdx);
        elemMapping.map(elemBody->getArgument(numElemInputs), outInit);
        ...
        Value fusedResult =
            elemMapping.lookupOrDefault(elemYield.getOperand(0));

        // Store fused result into outMem.
        b.create<memref::StoreOp>(loc, fusedResult, outMem, elemOutIdx);
        return;
      }
      if (pidx < forallRank) {
        // Already covered by the forall IVs.
        emitExtraParallelLoops(b, pidx + 1);
        return;
      }
      // Extra sequential loop for parallel dim beyond 3.
      int64_t dim = parallelDims[pidx];
      b.create<scf::ForOp>(
          loc, c0, bounds[dim], c1, ValueRange{},
          [&](OpBuilder &ib, Location, Value iv, ValueRange) {
            ivs[dim] = iv;
            emitExtraParallelLoops(ib, pidx + 1);
            ib.create<scf::YieldOp>(loc);
          });
    };

    emitExtraParallelLoops(builder, 0);
  }

  // ── Convert outMem back to tensor for SSA users ───────────────────────────
  auto outTensorType = RankedTensorType::get(outShape, elemType);
  Value fusedResult = builder.create<bufferization::ToTensorOp>(
      loc, outTensorType, outMem, /*restrict=*/true);
...
struct NovaLinalgVerticalFusionPass
  ...
  void runOnOperation() override {
    ...
    func.walk([&](linalg::GenericOp op) {
      ...
      auto bodyContainsLinalgIndex = [](linalg::GenericOp g) {
        return g.getBody()->walk([](linalg::IndexOp) {
          return WalkResult::interrupt();
        }).wasInterrupted();
      };
      if (bodyContainsLinalgIndex(op) || bodyContainsLinalgIndex(parallelOp))
        return;
