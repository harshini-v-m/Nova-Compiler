...
struct NovaOpTosaOp {
  ...
  //       auto newTargetsType =
  //       mlir::RankedTensorType::get(targetsType.getShape(),
  //                                                         builder->getI32Type());
  //       targets =
  //           builder->create<tosa::CastOp>(op.getLoc(), newTargetsType,
  //           targets);
  ...
  //       auto newLogitsType =
  //           mlir::RankedTensorType::get(logitsType.getShape(),
  //           targetElemType);
  //       logits =
  //           builder->create<tosa::CastOp>(op.getLoc(), newLogitsType,
  //           logits);
  ...
  //     maxShape[lastDim] = 1;
  //     auto maxValType = mlir::RankedTensorType::get(maxShape,
  //     targetElemType);

  //     Value maxVal = builder->create<tosa::ReduceMaxOp>(op.getLoc(),
  //     maxValType,
  //                                                       logits, axisAttr);

  //     // Step 2: z_shifted = logits - max_val
  //     Value zShifted = builder->create<nova::SubOp>(op.getLoc(), logits,
  //     maxVal);

  //     // Step 3: exp_z_shifted = exp(z_shifted)
  //     Value expZShifted = builder->create<nova::ExpOp>(op.getLoc(),
  //     zShifted);

  //     // Step 4: sum_exp = reduce_sum(exp_z_shifted, dim=-1, keepdims=true)
  //     Value sumExp = builder->create<tosa::ReduceSumOp>(op.getLoc(),
  //     maxValType,
  //                                                       expZShifted,
  //                                                       axisAttr);
  ...
  //         builder->create<nova::GatherOp>(op.getLoc(), logSmZ, targets,
  //         lastDim)
  //             .getResult();
  ...
  //         constType, builder->getFloatAttr(targetElemType, -1.0));
  //     Value minus1 =
  //         builder->create<nova::ConstantOp>(op.getLoc(), constType,
  //         minus1Attr);

  //     // Multiply selected_log_probs by -1
  //     Value negLogProbs =
  //         builder->create<nova::MulOp>(op.getLoc(), selectedLogProbs,
  //         minus1);
  ...
  //     llvm::SmallVector<int64_t> newShape = {1};
  //     auto shapeAttrType = RankedTensorType::get({1},
  //     builder->getIndexType()); auto shapeAttr =
  //     DenseIntElementsAttr::get(shapeAttrType, newShape);

  //     Value shapeConst = builder->create<tosa::ConstShapeOp>(
  //         op.getLoc(), mlir::tosa::shapeType::get(builder->getContext(), 1),
  //         shapeAttr);

  //     Value finalLoss = builder->create<tosa::ReshapeOp>(op.getLoc(),
  //     resultType,
  //                                                        reducedLoss,
  //                                                        shapeConst);
