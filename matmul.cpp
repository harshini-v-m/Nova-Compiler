...
// ============================================================================
static LogicalResult lowerScalarMatmul(Operation *op,
                                       ...
                                       Value resultMemRef,
                                       Value biasFlat = nullptr) {
  Location loc = op->getLoc();
  Type lhsElemType = cast<MemRefType>(lhsFlat.getType()).getElementType();
  int64_t tileSize = 16;
  int64_t elemByteSize = lhsElemType.getIntOrFloatBitWidth() / 8;
  ...
  {
    Value valueToStore = finalResult;
    if (biasFlat) {
      Value biasVal = rewriter.create<memref::LoadOp>(loc, biasFlat, ValueRange{col});
      if (llvm::isa<FloatType>(resultType.getElementType()))
        valueToStore = rewriter.create<arith::AddFOp>(loc, finalResult, biasVal);
      else
        valueToStore = rewriter.create<arith::AddIOp>(loc, finalResult, biasVal);
    }
    ...
    rewriter.create<memref::StoreOp>(loc, valueToStore, resFlat, ValueRange{rLinear});
  }
...
// ============================================================================
static LogicalResult lowerTensorCoreMatmul(Operation *op,
                                           PatternRewriter &rewriter,
                                           Value lhsFlat, Value rhsFlat,
                                           Value resFlat,
                                           int64_t M, int64_t K, int64_t N,
  ...
  auto issueStageLoad = [&](OpBuilder &b, Value stageIdx, Value kBase) {
    ...
    for (int64_t i = 0; i < A_PASSES; ++i) {
      ...
      // Use cg only for vectorized path; boundary fallback uses ca for stability.
      ...
      tokens.push_back(b.create<nvgpu::DeviceAsyncCopyOp>(
          ...
          /*bypassL1=*/useVecCopy ? rewriter.getUnitAttr() : nullptr));
    }
    ...
    for (int fj = 0; fj < (int)FR_N; ++fj) {
      ...
      for (int fr = 0; fr < 2; ++fr) {
        ...
        if (tileAligned) {
          ...
          // Add bias BEFORE truncation (bias is in accumulator precision)
          if (biasFlat) {
            auto biasVecTy = VectorType::get({2}, mmaAccTy);
            Value biasVec = rewriter.create<vector::LoadOp>(loc, biasVecTy, biasFlat, ValueRange{oC});
            rowVec = rewriter.create<arith::AddFOp>(loc, rowVec, biasVec);
          }
          ...
          for (int fc = 0; fc < 2; ++fc) {
            ...
            // Add bias BEFORE truncation
            if (biasFlat) {
              Value oCfc_bias = rewriter.create<arith::AddIOp>(loc, oC, ci(fc));
              Value biasVal = rewriter.create<memref::LoadOp>(loc, biasFlat, ValueRange{oCfc_bias});
              e = rewriter.create<arith::AddFOp>(loc, e, biasVal);
            }
            Value tW = e;
            if (resultType.getElementType() != mmaAccTy &&
                llvm::isa<FloatType>(resultType.getElementType()))
...
// ============================================================================
// Shared helper: buffer preparation + three-way dispatch
// ============================================================================
static LogicalResult dispatchMatmul(Operation *op,
                                    PatternRewriter &rewriter,
                                    Value lhs, Value rhs,
                                    RankedTensorType resultType,
                                    Value biasFlat = nullptr) {
    ...
    if (elemType.isF64()) {
      return lowerScalarMatmul(op, rewriter, lhsFlat, rhsFlat, resFlat,
                               ...
                               resultType, resultShape, resultMemRef, biasFlat);
      ...
      return lowerTensorCoreMatmul(op, rewriter, lhsFlat, rhsFlat, resFlat,
                                   ...
                                   rhsBatchStride, resBatchStride,
                                   resultType, resultShape, resultMemRef,
                                   /*useTF32=*/false, biasFlat);
    }
}

// ============================================================================
// Pattern: nova.matmul → GPU matmul kernel (no bias)
// ============================================================================
struct NovaToGpuMatmulPattern : public OpRewritePattern<nova::MatmulOp> {
  ...
  LogicalResult matchAndRewrite(nova::MatmulOp op,
    ...
    return dispatchMatmul(op, rewriter, op.getLhs(), op.getRhs(),
                          resultType, /*biasFlat=*/nullptr);
  }
};

// ============================================================================
// Pattern: nova.linear → GPU matmul kernel with bias fused in epilogue
// ============================================================================
struct NovaToGpuLinearPattern : public OpRewritePattern<nova::LinearOp> {
  using OpRewritePattern<nova::LinearOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(nova::LinearOp op,
                                PatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    ...
    // Flatten bias to 1D memref on device (address space 1)
    auto i1Attr = rewriter.getI64IntegerAttr(1);
    int64_t N = biasType.getShape().back();
    auto biasMemRefType = MemRefType::get(
        biasType.getShape(), biasType.getElementType(),
        MemRefLayoutAttrInterface{}, i1Attr);
    auto biasFlatType = MemRefType::get(
        {N}, biasType.getElementType(),
        MemRefLayoutAttrInterface{}, i1Attr);

    Value biasMemRef = rewriter.create<bufferization::ToBufferOp>(
        loc, biasMemRefType, bias).getResult();
    Value biasFlat = rewriter.create<memref::ReinterpretCastOp>(
        loc, biasFlatType, biasMemRef, (int64_t)0,
        ArrayRef<int64_t>{N}, ArrayRef<int64_t>{1});

    return dispatchMatmul(op, rewriter, op.getInput(), op.getWeight(),
                          resultType, biasFlat);
  }
};
...
void populateMatmulPatterns(RewritePatternSet &patterns) {
  ...
  patterns.add<NovaToGpuLinearPattern>(patterns.getContext());
}
