...
static GemmCutoffs getGemmCutoffs(const NVIDIATargetInfo &target) {
  if (target.maxWorkgroupMemBytes >= 100 * 1024) return {2, 16, 64};
...
sortMMAIntrinsics(const ContractProblem &problem,
  ...
  bool preferMmaSync = (target.preferredSubgroupSize == 32) &&
                       (target.maxWorkgroupMemBytes >= 100 * 1024); // sm >= 80
  ...
  for (const NVMMAIntrinsicInfo &intrinsic : sortedIntrinsics) {
    ...
    std::optional<GPUMMASchedule> fitted =
        fitScheduleInSharedMemory(sched, intrinsic,
                                  target.maxWorkgroupMemBytes, doCPromotion);
    if (!fitted) continue;
    sched = *fitted;
...
getMmaSchedule(const NVIDIATargetInfo &target, ContractProblem &problem,
  ...
  return deduceMMASchedule(problem, target, intrinsics, *maybeSeeds,
                           mustBeAligned, doCPromotion, splitReductionTripCnt);
}
...
// Reject ops where all indexing maps are permutations (no broadcast).
  ...
  // promotion before enabling this path.
  ...
  // Send very skinny matmuls ({2-4}xNxK or Mx{2-4}xK) to the vector
  // reduction pipeline, similar to matvec. Note: reassociation in that
  // pipeline may cause precision loss; hide behind a flag if needed.
  ...
  // Compute both promotion flags here and pass them downstream.
  // promotePrologueOperands: prologue ops feed non-matmul data into the kernel
  //   → C must be in shared memory so the epilogue can read/write it safely.
  // doCPromotion: accumulator buffer is live (matmul_accumulate)
  //   → kernel must load existing C before accumulating.
  ...
  // MMA path: arithmetic-intensity-driven seeds + sqrt/GCD/min distribution.
