...
module {
  func.func @main(%arg0: tensor<1024x1024xf32, #nova.device<"1">>, %arg1: tensor<1024x1024xf32, #nova.device<"1">>, %arg2: tensor<1024x1024xf32, #nova.device<"1">>) -> tensor<1024x1024xf32, #nova.device<"1">> attributes {llvm.emit_c_interface} {
    ...
    %4 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%2, %arg2 : tensor<1024x1024xf32, #nova.device<"1">>, tensor<1024x1024xf32, #nova.device<"1">>) outs(%3 : tensor<1024x1024xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %7 = arith.addf %in, %in_2 : f32
      linalg.yield %7 : f32
    } -> tensor<1024x1024xf32>
    %cast = tensor.cast %4 : tensor<1024x1024xf32> to tensor<1024x1024xf32, #nova.device<"1">>
    %5 = tensor.empty() : tensor<1024x1024xf32>
    %6 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%cast, %cst : tensor<1024x1024xf32, #nova.device<"1">>, tensor<1024x1024xf32, #nova.device<"1">>) outs(%5 : tensor<1024x1024xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %7 = arith.maximumf %in, %in_2 : f32
      linalg.yield %7 : f32
    } -> tensor<1024x1024xf32>
    %cast_1 = tensor.cast %6 : tensor<1024x1024xf32> to tensor<1024x1024xf32, #nova.device<"1">>
    return %cast_1 : tensor<1024x1024xf32, #nova.device<"1">>
  }
}
